...
int game_level[] = { 15,12,8 };//this sets the difficulty -> speed
float rate = game_level[level];

GLfloat brick_color_array[][3] = { {1,0,0},{0,0,1},{0,1,0},{1,0,1},{1,1,0},{0,1,1} }; //diff colours of brick - Red, Blue, Green, Purple, Yellow, Cyan 
...
void draw_paddle() // Function to draw the paddle
{
...
void brick(GLfloat x, GLfloat y, GLfloat z) //Function to draw a single brick
{
	glDisable(GL_LIGHTING);
...
void draw_bricks() // Function to draw the grid of bricks by setting brick matrix
{
	...
	if (start == 0)
	{
		for (i = 1; i <= rows; i++)
		{
			for (j = 1; j <= columns; j++)
			{
				...
				brick_array[i][j].x = (GLfloat)(j * 4 * 0.82);
...
void draw_ball() //Function to draw the spherical ball
{
	...
	glPushMatrix();
	glTranslatef(bx, by - 0.3, 0);
	glScalef(1.0, 1.0, 0.5);
	glutSolidSphere(0.8, 52, 52);
...
void mousemotion(int x, int y) //mouse function
{
...
void addMenu() //add menu to the screen
{
...
void text(int sc)  //Function to print the credits, instructions and score on the screen
{
	glDisable(GL_LIGHTING);
	char text[40];
	char difficulty[10];
	...
	glRasterPos3f(-0.7, 1, 20);
	char logo[70] = "UCS505 BREAKOUT GAME  :  Nikhil Katiyar(101803183)";
	for (int i = 0; logo[i] != '\0'; i++)
		glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, logo[i]);

	glRasterPos3f(1.1, 0.8, 20);
	char logo1[30] = "Mansimran Singh(101803181)";
	for (int i = 0; logo1[i] != '\0'; i++)
		glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, logo1[i]);

	glRasterPos3f(1.1, 0.6, 20);
	char logo2[30] = "Divjot Singh(101803177)";
	for (int i = 0; logo2[i] != '\0'; i++)
		glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, logo2[i]);

	glRasterPos3f(-0.3, 0.3, 20);
	char instructions[70] = "Controls:  s->Start; q-> Quit; a,d-> MovePaddle; RMB-> Menu ";
